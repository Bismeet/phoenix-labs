...
const GhostCursor: React.FC<GhostCursorProps> = ({
  ...
  // ---------- EXACT original React Bits shaders ----------
  ...
  const fragmentShader = `
    ...
    vec3 tint1(vec3 base){ return mix(base, vec3(1.0, 0.6, 0.1), 0.35); }
    vec3 tint2(vec3 base){ return mix(base, vec3(0.8, 0.2, 0.05), 0.4); }

    vec4 blob(vec2 p, vec2 mousePos, float intensity, float activity) {
      vec2 pScaled = p * 1.5;
      vec2 mouseScaled = mousePos * 1.5;

      vec2 q = vec2(fbm(pScaled * iScale + iTime * 0.15), fbm(pScaled * iScale + vec2(5.2,1.3) + iTime * 0.12));
      vec2 r = vec2(fbm(pScaled * iScale + q * 1.5 + iTime * 0.2), fbm(pScaled * iScale + q * 1.5 + vec2(8.3,2.8) + iTime * 0.18));

      float smoke = fbm(pScaled * iScale + r * 0.8);
      float radius = 0.22 + 0.10 * (1.0 / iScale);
      ...
      float flicker = sin(iTime * 0.5 + smoke * 6.0) * 0.5 + 0.5;
      vec3 color = mix(c2, c1, distFactor * flicker);
  ...
  useEffect(() => {
    ...
    // Use viewport dimensions for the renderer (fixed positioning)
    ...
    // Resize to viewport since we use position: fixed
    const resize = () => {
      if (!active) return;
    ...
    // Track mouse relative to viewport (since canvas is fixed/viewport-sized)
